...
# gs load a graph
graph = gs.import_onnx(onnx.load("../yolov5/yolov5s.onnx"))
...
inputs = [tensors["output"].to_variable(dtype=np.float32), 
    tensors["327"].to_variable(dtype=np.float32), 
    tensors["328"].to_variable(dtype=np.float32)]

# Add a output tensor of new graph
modified_output = gs.Variable(name="output0", dtype=np.float32, shape=(6001, 1, 1))
...
# gs save a graph
onnx.save(gs.export_onnx(graph), "yolov5s_yolo_op.onnx")
